...
plot_simulation = function(data, name) {
  ...
  g = ggplot(data = data,
    ...
    annotate('text', color = 'red3',
             label = sprintf('r = %s\nRMSE = %.2f', r, rmse),
             hjust = 0,   # left align
             x = limits[1], y = limits[2] - 25) +
...
plot_cause_vs = function(model_name) {
  ...
  g = ggplot(data = data_pred,
    ...
    annotate('text',
             label = sprintf('r = %s\nRMSE = %.2f', r, rmse),
             hjust = 0,
             x = limits[1], y = limits[2] - 2) +
    scale_x_continuous(name = case_when(
      model_name == 'cf' ~ 'counterfactual contrast judgment',
      model_name == 'hyp' ~ 'hypothetical contrast judgment',
